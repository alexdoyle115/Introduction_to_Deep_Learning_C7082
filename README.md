...
Visualing the various layers of the convoltional neural network offers insight into what the "black box" that is a neural network is actually picking up on. The `Rust_identifier_1` model that was trained earlier in the analysis is reloaded as well as the `models`package in `keras`. The image below in Fig. 7 which is a leaf infected with stripes of rust while Fig 9 is an image that is nitrogen deficent. Ideally the model should see some activity in these regions. 
